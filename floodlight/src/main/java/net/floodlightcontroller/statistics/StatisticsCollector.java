...
public class StatisticsCollector implements IFloodlightModule,
	...
	private static int portStatsInterval = 1; /*
											 * could be set by REST API, so not
											 * final
											 */
	...
	private class PortStatsCollector implements Runnable {
		...
		public void run() {
			...
			for (Entry<NodePortTuple, SwitchPortBandwidth> entry : portStats
					.entrySet()) {
				Long bytesPerSecondRx = entry.getValue().getBitsPerSecondRx()
						.getValue() / 8;
				Long bytesPerSecondTx = entry.getValue().getBitsPerSecondTx()
						.getValue() / 8;

				if (bytesPerSecondRx > 2500000 || bytesPerSecondTx > 2500000) {
					log.info("ELEPHANT FLOW");
					log.info("[STATISTICS] Measuring on switch={} and port={}",
							entry.getKey().getNodeId(), entry.getKey()
									.getPortId());
					log.info(
							"[STATISTICS] Bandwidth consumption={} bits/s RX, {} bits/s TX",
							bytesPerSecondRx, bytesPerSecondTx);
				}
			}
		}
	}
	...
	private void startStatisticsCollection() {
		...
		flowStatsCollector = threadPoolService.getScheduledExecutor()
				.scheduleAtFixedRate(new FlowStatsCollector(),
						portStatsInterval, portStatsInterval, TimeUnit.SECONDS);
	...
	private void stopStatisticsCollection() {
		if (!portStatsCollector.cancel(false)
				|| !flowStatsCollector.cancel(false)) {
	...
	protected class FlowStatsCollector implements Runnable {
		...
		private List<Long> previousByteCount = new ArrayList<>(Arrays.asList(0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 
				0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L));
		
		@Override
		public void run() {
			log.info("FLOW STATS COLLECTOR RUN");
			flowStats.clear(); // to clear expired flows
			Map<DatapathId, List<OFStatsReply>> replies = getSwitchStatistics(
					switchService.getAllSwitchDpids(), OFStatsType.FLOW);
			for (Entry<DatapathId, List<OFStatsReply>> e : replies.entrySet()) {
				if (!e.getKey().equals(DatapathId.of("00:00:00:00:00:00:00:01"))) {
					continue;
				}
				for (OFStatsReply r : e.getValue()) {
					OFFlowStatsReply psr = (OFFlowStatsReply) r;
					for (OFFlowStatsEntry pse : psr.getEntries()) {
						Pair<Match, DatapathId> pair = new Pair<>(
								pse.getMatch(), e.getKey());
						flowStats.put(pair, FlowRuleStats.of(e.getKey(),
								pse.getByteCount(), pse.getPacketCount(),
								pse.getPriority(), pse.getHardTimeout(),
								pse.getIdleTimeout(), pse.getDurationSec()));

					}
				}
			}
			int i = 0;
			for (Entry<Pair<Match, DatapathId>, FlowRuleStats> entry : flowStats
					.entrySet()) {
				DatapathId datapathId = entry.getKey().getValue();
				if (!datapathId.equals(DatapathId.of("00:00:00:00:00:00:00:01"))) {
					continue;
				}
			
				Long duration = entry.getValue().getDurationSec();
				Long byteCount = entry.getValue().getByteCount().getValue();
				Match match = entry.getKey().getKey();
				
				if (byteCount - this.previousByteCount.get(i) >= 2500000 && duration >= 10) {
					log.info("ELEPHANT FLOW CATCHED ON DATAPATHID={} AND MATCH={}", datapathId, match.toString());
					log.info("Duration: {} | ByteCount/s: {}", duration, byteCount - this.previousByteCount.get(i));
					Flows.rerouteElephant();
				}
				
				if (this.previousByteCount.get(i) != 0L) {
					log.info("Previous: {}", this.previousByteCount.get(i));
				}
				
				this.previousByteCount.set(i, byteCount);
				i++;
			}
		}
	}
